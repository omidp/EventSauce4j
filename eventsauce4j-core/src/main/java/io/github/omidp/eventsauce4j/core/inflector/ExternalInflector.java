...
package io.github.omidp.eventsauce4j.core.inflector;
 ...
 */
public class ExternalInflector implements Inflector {

	private Map<String, Class<?>> mapping = new HashMap<>();

	public ExternalInflector(String... packages) {
		this(List.of(packages));
	}

	public ExternalInflector(Collection<String> packages) {
		ClassPathScanningCandidateComponentProvider provider =
			new ClassPathScanningCandidateComponentProvider(false);
		provider.addIncludeFilter(new AnnotationTypeFilter(ExternalEvent.class));
		for (String pkg : packages) {
			for (BeanDefinition candidate : provider.findCandidateComponents(pkg)) {
				String className = candidate.getBeanClassName();
				try {
					Class<?> clazz = Class.forName(className);
					if (clazz.isAnnotationPresent(ExternalEvent.class)) {
						ExternalEvent clazzAnnotation = clazz.getAnnotation(ExternalEvent.class);
